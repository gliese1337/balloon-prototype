...
function update_macros(max: number, streamed: Float32Array, macros: Float32Array) {
  for (let i = 0, im = 0; i < max; i++, im += 3) {
    let rho = streamed[i];  // macroscopic density
...
export default class LatticeBoltzmann {        
  ...
  public step(viscosity: number, barriers: boolean[]) {
    const { xdim, ydim, macros, collided, streamed } = this;
    const max = xdim * ydim;
